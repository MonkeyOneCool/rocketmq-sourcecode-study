...
//        int queueId msg.getQueueId();
        ...
        //这里会对将消息存储到CommitLog文件的操作进行加锁，默认是ReentrantLock
    ...
    public CompletableFuture<PutMessageStatus> submitFlushRequest(AppendMessageResult result, MessageExt messageExt) {
        ...
        // Asynchronous flush
            ...
            if (!this.defaultMessageStore.getMessageStoreConfig().isTransientStorePoolEnable()) {
                //普通处理方式（FlushRealTimeService）
                flushCommitLogService.wakeup();
            } else {
                //堆外内存处理方式（CommitRealTimeService）
    ...
    class CommitRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                try {
                    //将待提交数据提交到内存映射共享区中（内部会使用堆外内存的方式）
    ...
    class FlushRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                try {
                    //刷盘前先等待，flushCommitLogTimed为true则使用Thead.sleep的方式休眠；flushCommitLogTimed为false则使用wait（CountDownLatch2）的方式休眠，
        ...
        private void doCommit() {
            if (!this.requestsRead.isEmpty()) {
                for (GroupCommitRequest req : this.requestsRead) {
                    ...
                    //是否刷盘成功，调用CompletableFuture.complete方法将刷盘结果放进去
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            /*
            如果消息长度大于CommitLog文件的最大空闲空间，就返回AppendMessageStatus.END_OF_FILE
            之后会重新创建一个新的CommitLog文件，并重新调用本方法进行追加消息
             */
            ...
            //pos为当前消息的总长度，下面就是在计算pos值，根据不同pos值的位置存放相应的值
            int pos = 4 + 4 + 4 + 4 + 4;
            // 6 QUEUEOFFSET
            //当前消息在消息队列中的消费偏移量
            ...
            //将消息内容存储到ByteBuffer中（这里只是将消息存储在MappedFile对应的内存映射Buffer中，并没有刷写到磁盘中）
