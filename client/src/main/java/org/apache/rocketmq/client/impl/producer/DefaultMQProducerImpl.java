...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            //如果生产者中没有缓存topic的路由信息，又或者没有消息队列，则重新从NameServer中查询，并更新topicPublishInfoTable
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                if (!(msg instanceof MessageBatch)) {
                    //如果不是批量发送消息的话，在这里会为消息分配全局唯一ID，批量发送消息的ID在其内部逻辑中已经被设置了
                ...
                switch (communicationMode) {
                    /*
                    异步发送和单向发送都是向Netty直接发送网络请求，然后直接返回null（异步发送会在
                    拿到结果后调用callback回调函数）。而同步发送会去等待请求Netty的结果并处理返回值
                     */
                    case ASYNC:
